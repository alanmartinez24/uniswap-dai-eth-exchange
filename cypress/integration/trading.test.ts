describe('Swap', () => {
  ...
  it('should have zero amounts in both inputs at the beginning', async () => {
  ...
  it('should show a button for maximum amount of DAI', async () => {
    cy.get('.crypto-input-wrapper-DAI .btn-swap-max-dai').should('be.visible')
  })

  it('should update the amount of ETH when the amount of DAI is updated', async () => {
    cy.get('.crypto-input-DAI').clear().type('1000')
    cy.get('.crypto-input-ETH').should('not.have.value', '0.0')
  })

  it('should update the amount of DAI when the amount of ETH is updated', async () => {
    cy.get('.crypto-input-ETH').clear().type('1.0')
    cy.get('.crypto-input-DAI').should('not.have.value', '0.0')
  })

  it('should show error message when zero amount of DAI is being swapped', async () => {
